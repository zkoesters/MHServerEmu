...
`PlayerManager.DatabaseType` selects account/player storage and can be `Json`, `SQLite`, `PostgreSQL`, or `MySQL`. `Leaderboards.DatabaseType` independently selects leaderboard storage and can be `SQLite`, `PostgreSQL`, or `MySQL`. Account/player data and leaderboards can each use a supported backend independently. `UseJsonDBManager` is a legacy setting; leave it `false` when selecting PostgreSQL.
...
For MySQL/MariaDB setup, see [MySQL and MariaDB Setup](./MySQL.md).

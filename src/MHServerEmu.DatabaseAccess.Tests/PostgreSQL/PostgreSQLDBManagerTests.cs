...
namespace MHServerEmu.DatabaseAccess.Tests.PostgreSQL
{
    public class PostgreSQLDBManagerTests
    {
        ...
        {
            using NpgsqlCommand command = new(@"INSERT INTO player (db_guid, archive_data, start_target, aoi_volume, gazillionite_balance, last_logout_time)
                VALUES (@DbGuid, @ArchiveData, @StartTarget, @AOIVolume, @GazillioniteBalance, @LastLogoutTime)", connection);

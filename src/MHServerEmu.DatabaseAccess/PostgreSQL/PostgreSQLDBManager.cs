    ...
    public class PostgreSQLDBManager : IDBManager
    {
        ...
        public bool LoadPlayerData(DBAccount account)
        {
            ...
            try
            {
                ...
                account.Player = connection.QueryFirstOrDefault<DBPlayer>(@"SELECT db_guid AS DbGuid, archive_data AS ArchiveData,
                    ...
                    last_logout_time AS LastLogoutTime FROM player WHERE db_guid = @DbGuid", new { DbGuid = account.Id }, transaction);
        ...
        private void SavePlayerDataAttempt(DBAccount account)
        {
            ...
            try
            {
                connection = GetConnection();
                transaction = connection.BeginTransaction();
                connection.Execute(@"INSERT INTO player (db_guid, archive_data, start_target, aoi_volume, gazillionite_balance, last_logout_time)
                    VALUES (@DbGuid, @ArchiveData, @StartTarget, @AOIVolume, @GazillioniteBalance, @LastLogoutTime)
                    ...
                    last_logout_time = EXCLUDED.last_logout_time", account.Player, transaction);
